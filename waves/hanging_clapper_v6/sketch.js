...
var bellSketch = function(s) {
  ...
  s.draw = function() {
    ...
    if (p.velocity < 0) {
      p.velocity += Math.abs(s.accelerationX) * -1 * sensitivity; }
    else  {
      p.velocity += Math.abs(s.accelerationX)* sensitivity; }

  }

  s.collision = function() {
      s.ring(Math.abs(p.velocity) / 150.0); // ring based on value of velocity
  ...
  s.initSound = function() {
    ...
    for (var i = 0; i < numOsc; i++) {
        randFreqs[i] = s.random(100, 12000);
    }
    ...
    for (i = 0; i < numOsc; i++) {
      ...
      o.detune.value = s.random(-4, +4); // random detuning
